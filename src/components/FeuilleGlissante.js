...
// (aperçu), on tire la poignée vers le haut pour l'agrandir en plein écran,
// vers le bas pour la refermer. Le contenu (ContenuProfil) ne change pas
// entre les deux états — seule la hauteur visible varie, comme sur un vrai
// bottom sheet. Bloque le défilement de la page pendant qu'elle est ouverte
// (même principe que Visionneuse.js) : la liste derrière reste IMMOBILE
// pendant le geste, pour ne pas reproduire le bug GPU de la tabbar (couche
...
export default function FeuilleGlissante({ children, onFermer }) {
  ...
  useEffect(() => {
    const poignee = feuilleRef.current?.querySelector(".fg-poignee");
    const f = feuilleRef.current;
    if (!poignee || !f) return;
    ...
    poignee.addEventListener("pointerdown", descendre);
    ...
    return () => {
      poignee.removeEventListener("pointerdown", descendre);
