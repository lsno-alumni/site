...
// Liste COMPLÈTE des pays/territoires reconnus (253) — générée depuis les
// vrais codes ISO via `Intl.DisplayNames`, régénérable avec
// `node outils/gen_liste_pays.js` si le paquet `flag-icons` est mis à jour.
// Choisie exclusivement par recherche (ChoixPays.js) : jamais de texte
// libre, donc toujours un vrai code → toujours un drapeau.
...
// Un drapeau par code ISO (SVG, package `flag-icons`, copiés dans
// public/img/drapeaux/ par outils/gen_liste_pays.js).
export const PAYS = Object.fromEntries(
  LISTE_PAYS.map(([code, nom]) => [code, { nom, drapeau: `/img/drapeaux/${code.toLowerCase()}.svg` }])
);
